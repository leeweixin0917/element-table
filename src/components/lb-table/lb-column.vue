...
<div>
  ...
  <el-table-column v-if="column.prop === 'operation'" :key="column.index" align="center" :prop="column.prop" :label="column.label">
	  <!--if判断的是父组件传的表头是操作的id名-->
	  <template slot-scope="scope">
	      <el-button v-for="item1 in column.list" :key="item1.id" @click="examine(scope.row, item1.id)" type="text" size="small">{{item1.name}}</el-button>
